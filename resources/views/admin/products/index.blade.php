								...
								<tbody>
									@forelse ($products as $row)
										<tr>
											<td>
												@if ($row->image)
													<img src="{{ route('admin.document.download', ['type' => 'product-image', 'id' => $row->id, 'action' => 'view']) }}" alt="" width="60">
												@else
											...
											<td>
												<a class="btn btn-sm btn-info"
													href="{{ route('admin.products.edit', $row) }}">Edit</a>
												<button type="button" class="btn btn-sm btn-danger btn-swal-delete"
													data-form-id="delete-form-product-{{ $row->id }}">Delete</button>
												<form id="delete-form-product-{{ $row->id }}" class="d-none"
													method="POST"
													action="{{ route('admin.products.destroy', $row) }}">
													@csrf
													@method('DELETE')
												</form>
											</td>
